...
#define DECLARE_SERIALIZER_CALL(request_type) \
	std::unique_ptr<iserialized> request_type::serialize(iserializer &) { \
		return iserializer.serialize(std::move(*this)); \
	}

DECLARE_SERIALIZER_CALL(dnet_read_request)
DECLARE_SERIALIZER_CALL(dnet_read_response)
DECLARE_SERIALIZER_CALL(dnet_write_request)
DECLARE_SERIALIZER_CALL(dnet_lookup_response)
