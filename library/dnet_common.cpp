...
extern char *dnet_backend_command_strings[];
...
int dnet_copy_addrs_nolock(struct dnet_net_state *nst, struct dnet_addr *addrs, int addr_num)
{
	...
	nst->addrs = static_cast<dnet_addr*>(malloc(sizeof(struct dnet_addr) * addr_num));
	if (!nst->addrs) {
		err = -ENOMEM;
		goto err_out_exit;
	}
...
{
	struct dnet_net_state *st = static_cast<dnet_net_state*>(state);
...
{
	const struct dnet_weight *w1 = static_cast<const dnet_weight*>(v1);
	const struct dnet_weight *w2 = static_cast<const dnet_weight*>(v2);
...
int dnet_mix_states(struct dnet_session *s, struct dnet_id *id, uint32_t ioflags, int **groupsp)
{
	...
	weights = static_cast<dnet_weight*>(alloca(s->group_num * sizeof(*weights)));
	groups = static_cast<int*>(malloc(s->group_num * sizeof(*groups)));
	if (groups)
...
static int dnet_data_map_ll(struct dnet_map_fd *map, int prot)
{
	...
	map->data = static_cast<char*>(map->mapped_data) + map->offset - off;
		...
		for (it = rb_first(&st->idc_root); it; it = rb_next(it)) {
			...
			*entries = tmp_entries;
			...
			DNET_LOG(n, DNET_LOG_DEBUG, "%s: %s, group: %d, backend: %d, idc: %p",
				...
				idc->group->group_id, idc->backend_id, static_cast<void*>(idc));
		}
...
{
	const dnet_iterator_response *a = static_cast<const dnet_iterator_response*>(r1);
	const dnet_iterator_response *b = static_cast<const dnet_iterator_response*>(r2);
...
{
	dnet_map_fd map;
	memset(&map, 0, sizeof(dnet_map_fd));
	map.fd = fd;
	map.size = size;
...
{
	dnet_map_fd left_map;
	memset(&left_map, 0, sizeof(dnet_map_fd));
	left_map.fd = left_fd;
	left_map.size = left_size;

	dnet_map_fd right_map;
	memset(&right_map, 0, sizeof(dnet_map_fd));
	right_map.fd = right_fd;
	right_map.size = right_size;
	...
	while (right_offset < right_size) {
		...
		if (left_offset < left_size && cmp <= 0) {
			 ...
			 */
			dnet_iterator_response_skip_equal_keys(static_cast<dnet_iterator_response*>(left_map.data),
				                               &left_offset, left_size);

			/* For same key we move both pointers */
			if (cmp_id == 0)
				dnet_iterator_response_skip_equal_keys(static_cast<dnet_iterator_response*>(right_map.data),
...
struct dnet_iterator *dnet_iterator_alloc(uint64_t id)
{
	...
	it = static_cast<dnet_iterator*>(calloc(1, sizeof(struct dnet_iterator)));
	if (it == NULL)
		goto err_out_exit;
...
int dnet_fd_readlink(int fd, char **datap)
{
	...
	dst = static_cast<char*>(malloc(dsize));
	if (!dst) {
		err = -ENOMEM;
		goto err_out_exit;
	}
