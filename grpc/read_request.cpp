...
read_request_manager::read_request_manager(service_provider_info *provider_in, THandler handler_in)
...
void read_request_manager::send_response(std::unique_ptr<TResponse> response) {
...
read_request_manager::TRpcResponse read_request_manager::serialize_next(const TResponse &response, bool &complete) {
	...
	if (!response_json_offset_ && !response_data_offset_) {
		...
		if (response.data.where() == ell::data_place::IN_MEMORY) {
