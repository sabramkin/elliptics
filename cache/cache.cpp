...
static int dnet_cmd_cache_io_write_new(struct cache_manager *cache,
	...
	if (status == write_status::HANDLED_IN_CACHE) {
		std::unique_ptr<dnet_lookup_response> response(new dnet_lookup_response);

		response->cmd = request.cmd;
		response->cmd.size = 0; // TODO: set size on serialize
		...
		err = dnet_send_common_response(st, std::move(response), context);
...
static int dnet_cmd_cache_io_read_new(struct cache_manager *cache,
                                      struct dnet_net_state *st,
                                      ioremap::elliptics::dnet_read_request &request,
                                      struct dnet_cmd_stats *cmd_stats,
                                      dnet_access_context *context) {
	using namespace ioremap::elliptics;
	...
	std::unique_ptr<dnet_read_response> response(new dnet_read_response);
	...
	// TODO: lifetime of json in cache memory = ? maybe not copy?
	...
	// TODO: lifetime of data in cache memory = ? maybe not copy?
	...
	return dnet_send_common_response(st, std::move(response), context);
}
